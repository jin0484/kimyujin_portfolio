 ...
 *   ① 남아 있던 초록 M(ME 의 M)이 바닥선으로 접혀 선이 되고, W 가 왔던 길을 거꾸로 — 바닥선을 따라 왼쪽 → 왼쪽 테두리를 타고 위로 →
 *      첫 가로선을 따라 오른쪽으로 달려가 Meaning 의 M 으로 펴진다 — 그림은 stage1.js renderWork 가 이 모듈의 sloganPhase·sloganM 을 보고 그린다.
...
const PAD = 20, GAP = 0.05, LINE = 158;                     // 왼쪽 여백(무대 px, WORK 의 W 와 같음) · M 그림과 e 사이(글자 크기 대비) · 앉는 가로선(격자 y — box.js HY2 의 첫 줄)
...
function metrics() {                                         // Chillax 치수 — 대문자 높이 · 줄 상자 윗변에서 기준선까지(line-height 1) · 나머지 글자 폭
  const g = document.createElement('canvas').getContext('2d');
  g.font = "500 100px 'Chillax'";
  const cap = g.measureText('M').actualBoundingBoxAscent / 100;
  const m = g.measureText(REST), asc = m.fontBoundingBoxAscent / 100, dsc = m.fontBoundingBoxDescent / 100;
  return { cap, base: (1 - (asc + dsc)) / 2 + asc, w: m.width / 100 };
}
function layout() {
  if (!fm) fm = metrics();
...
export const sloganM = () => { if (!lay) layout(); return lay; };   // 슬로건 M 자리 — 무대 좌표 x · 폭 w · 높이 h · 기준선(= 앉는 가로선) base
